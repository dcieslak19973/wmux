...
function workbookClientScript() {
  return [
    ...
    '      state.selectedRowIndex = 0; render(); break;',
    '    case "set_search":',
    '      state.search = cmd.value !== undefined ? String(cmd.value) : "";',
    '      state.selectedRowIndex = 0; render(); break;',
    '    case "select_row":',
    '      if (typeof cmd.index === "number") { state.selectedRowIndex = cmd.index; render(); } break;',
    '    case "select_group":',
    '      state.selectedGroup = cmd.group !== undefined ? cmd.group : null;',
    '      state.selectedRowIndex = 0; render(); break;',
    '    case "reset":',
    '      state.search = ""; state.filters = Object.create(null);',
    '      state.sortField = workbook.chart?.groupBy || workbook.columns[0] || "";',
    '      state.sortDir = "desc"; state.selectedGroup = null; state.selectedRowIndex = 0;',
    '      render(); break;',
    '  }',
    '};',
    'const pollCommands = () => {',
